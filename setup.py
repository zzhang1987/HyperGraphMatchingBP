...
FactorGraph_module = Extension('_FactorGraph',
                            sources=['FactorGraph.i','FactorGraph.cpp', 'FactorCreator.cpp', 'Factor.cpp', 'Auction.cpp'],
                            extra_compile_args=['-O3', '-I/usr/local/include','-DPERFECT_MATCHING_DOUBLE', '-ffast-math', '-pipe', '-fomit-frame-pointer', '-std=c++11', '-stdlib=libc++'],
                            swig_opts=['-modern','-c++']
