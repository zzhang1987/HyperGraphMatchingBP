function  [tend, id, res] = runRRWM(Fname)
load(Fname);
addpath('RRWM');
...
if exist('KQ','var')
    ...
    M = zeros(NofNodes*NofNodes,NofNodes*NofNodes);
    dialog_KP = KP.*eye(NofNodes);
    M = M + kron(dialog_KP,eye(NofNodes));
    E1 = Edges;
    E2 = NEdges;
    KQ = KQ/2;
    for i=1:size(E1,1)
        for j=1:size(E2,1)
            M(E2(j,1)+E1(i,1)*NofNodes+1,E2(j,2)+E1(i,2)*NofNodes+1) = KQ(i,j);
            M(E2(j,1)+E1(i,2)*NofNodes+1,E2(j,2)+E1(i,1)*NofNodes+1) = KQ(i,j);
        end
    end
    M = M + M';
...
tstart = tic;
[X] = RRWM(M,group1,group2);
tend = toc(tstart);
X = reshape(X,[NofNodes NofNodes]);
X = asgHun(X);
